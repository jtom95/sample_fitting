...
class DipoleFieldKernelExtractor:
    ...
    @property
    def x(self):
        return np.linspace(-self.dim_range / 2, self.dim_range / 2, self.num_points)
    ...
    def _calculate_dipole_main_field(
        ...
        if normalize:
            return dipole_field / np.max(dipole_field)
        return dipole_field
    ...
    def _calculate_dipole_1d_signal(
        ...
        if signal_type == "Ez":
            return self._calculate_dipole_main_field(
                base_dipole="Ez", axis="x", normalize=normalize
            )
        if signal_type == "Hy_x":
            return self._calculate_dipole_main_field(
                base_dipole="Hy", axis="x", normalize=normalize
            )
        if signal_type == "Hy_y":
            return self._calculate_dipole_main_field(
                base_dipole="Hy", axis="y", normalize=normalize
            )
    ...
    def evaluate_single_dipole_signal(
        ...
        if signal_type not in self.signals:
            self.signals[signal_type] = self._calculate_dipole_1d_signal(
